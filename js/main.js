const logo_cont = document.querySelector('header .logo');
const mode = document.querySelector('header .mode img');
const dark_mode = './assets/images/icon-moon.svg'
const light_mode = './assets/images/icon-sun.svg'
const dark = '#091540';
const light = '#ededed';
const Light_Gradient= 'linear-gradient(180deg, #EBF2FC 0%, #EEF8F9 100%)';
const Dark_Gradient= 'linear-gradient(180deg, #040918 0%, #091540 100%)';
const filters = document.querySelectorAll('.filter .filters > *');
let logo_color;
...
logo_cont.innerHTML = change_svg(logo_color);
...
function filter() {
      const extentions = document.querySelectorAll('.extentions .extention');
      filters.forEach(e => {
      ...
      chack_inactive(extentions, this.dataset.state);
      chack_all(extentions, this.dataset.state);
      chack_active(extentions, this.dataset.state);
}

function chack_active(extentions, filter) {
         if (filter == 'active') {
...
function chack_inactive(extentions, filter) {
         if (filter == 'inactive') {
            extentions.forEach(e => {
               e.style.display = 'flex';
               if (e.classList.contains('active')) {
...
function chack_all(extentions, filter) {
         if (filter == 'all') {
            extentions.forEach(e => {
               e.style.display = 'flex';
            })
...
function dealTheData(e) {
         const extention = document.createElement('div');
         const container = document.createElement('div');
         const content = document.createElement('div');
         const name = document.createElement('h3');
         const description = document.createElement('p');
         const icon_cont = document.createElement('div');
         ...
         if (e.isActive) {
            active.className = 'active-btn active';
            extention.className =  'extention active';
         }else {
            active.className = 'active-btn';
            extention.className =  'extention';
         }
         ...
         active.addEventListener('click', function () {
            ...
            extention.classList.toggle('active');
            document.querySelector('.filters .active').click();
         });
         ...
         icon_cont.appendChild(logo);
         content.appendChild(name);
         content.appendChild(description);
         container.appendChild(icon_cont);
         container.appendChild(content);
         extention.appendChild(container);
         extention.appendChild(control)
         document.querySelector('.extentions').appendChild(extention);
      }